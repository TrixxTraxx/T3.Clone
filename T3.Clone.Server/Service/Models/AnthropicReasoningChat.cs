...
public class AnthropicReasoningChat(
...
) : IChatModel
{
        ...
        Action<string> tokenCallback, Action<string> thinkingTokenCallback, Action<string> errorCallback)
    {
        try
        {
            // If no reasoning is requested, use the regular chat
            if (entity.ReasoningEffortLevel == ReasoningEffortLevel.None)
            {
                return await new AnthropicChat(dbContext, attachmentService, keyService)
